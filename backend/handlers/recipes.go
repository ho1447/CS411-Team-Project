...
// @Success     200 {object} api.RespMessage
// @Failure     500  {object}  api.RespMessage
// @Router      /save/remove/{userID}/{recipeID} [post]
func RemoveRecipesSaved(ctx *gin.Context) {
	client := api.NewClient(os.Getenv("RECIPE_API_KEY")) // get API key fron env var
	userID := ctx.Param("userID")
	recipeID := ctx.Param("recipeID")
	output, err := client.RemoveSavedRecipe(userID, recipeID)
	b, berr := json.Marshal(output.Message)
	if berr != nil {
		fmt.Println(berr)
		ctx.JSON(http.StatusInternalServerError, "")
	}
